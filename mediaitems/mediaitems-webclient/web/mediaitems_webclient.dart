...
class MetadataViewController {
  ...
  Iterable<MetadataItem> parseResult(String json) {
    List resultList = JSON.decode(json);
    return resultList.map((Map map) {
      return new MetadataItem(map["name"], map["mediaType"]);
    });
  }
...
class MetadataResultViewModel {
  int start = 0;
  int pagesize = 20;
  Iterable<MetadataItem> queryResult;
}
